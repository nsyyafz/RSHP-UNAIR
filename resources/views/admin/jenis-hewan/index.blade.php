        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-hover mb-0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            <span class="badge text-bg-primary">{{ $item->idjenis_hewan }}</span>
                                        </td>
                                        <td>
                                            <strong>{{ $item->nama_jenis_hewan }}</strong>
                                        </td>
                                        <td class="text-center">
                                            <div class="btn-group" role="group">
                                                <a href="{{ route('jenis-hewan.edit', $item->idjenis_hewan) }}" 
                                                   class="btn btn-warning btn-sm"
                                                   data-bs-toggle="tooltip" 
                                                   title="Edit">
                                                    <i class="bi bi-pencil-square"></i>
                                                </a>
                                                <form action="{{ route('jenis-hewan.destroy', $item->idjenis_hewan) }}" 
                                                      method="POST" 
                                                      style="display: inline;"
                                                      onsubmit="return confirm('Yakin ingin menghapus {{ $item->nama_jenis_hewan }}?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" 
                                    ...
                                    <tr>
                                        <td colspan="3" class="text-center py-5">
                                            <div class="text-muted">
                                                ...
                                                <p class="mt-3">Tidak ada data jenis hewan</p>
                                                <a href="{{ route('jenis-hewan.create') }}" class="btn btn-primary btn-sm">
                    ...
                    @if(count($jenisHewan) > 0)
                    <div class="card-footer clearfix">
                        <div class="float-end">
                            <span class="text-muted">Total: <strong>{{ count($jenisHewan) }}</strong> data</span>
                        </div>
                    </div>
                    @endif
                </div>
