    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-8 offset-md-2">
                <div class="card">
                    <div class="card-header"><h3 class="card-title">Form Edit Role User</h3></div>
                    <form action="{{ route('role.update', $roleUser->idrole_user) }}" method="POST">
                        @csrf @method('PUT')
                        <div class="card-body">
                            @if($errors->any())
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('iduser') is-invalid @enderror" 
                                    ...
                                    <option value="{{ $user->iduser }}" {{ old('iduser', $roleUser->iduser) == $user->iduser ? 'selected' : '' }}>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('idrole') is-invalid @enderror" 
                                    ...
                                    <option value="{{ $role->idrole }}" {{ old('idrole', $roleUser->idrole) == $role->idrole ? 'selected' : '' }}>
                                        {{ $role->nama_role }}
                                    </option>
                                    @endforeach
                                </select>
